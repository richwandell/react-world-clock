...
export default class OffsetSelector extends Component {
    ...
    static Times = [
        ...
        {
            "@value": "-4",
            "#text": "(GMT -4:00) Atlantic Time (Canada), La Paz"
        },
        {
            "@value": "-3.5",
    ...
    render() {
        ...
        const options = OffsetSelector.Times.map((t) => {
            let selected = this.state.selected === t["@value"] ? "selected" : "";
            return <option value={t["@value"]} {selected}>{t["#text"]}</option>;
        });
